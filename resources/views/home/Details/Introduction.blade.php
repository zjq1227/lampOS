					...
					<div class="clearfixLeft" id="clearcontent">
						...
						<div class="box">
							...
							<div class="tb-booth tb-pic tb-s310">
								<a href="{{asset("uploads")}}/{{$goods->picname}}"><img src="{{asset("uploads")}}/{{$goods->picname}}" alt="细节展示放大镜特效" rel="{{asset("uploads")}}/{{$goods->picname}}" class="jqzoom" /></a>
							</div>
							<ul class="tb-thumb" id="thumblist">
								@foreach ($goods->goodsimg as $goodsimg)
								<li class="tb-selected">
									<div class="tb-pic tb-s40">
									<a href="#"><img src="{{asset("uploads")}}/{{$goods->picname}}" mid="{{asset("uploads")}}/{{$goods->picname}}" big="{{asset("uploads")}}/{{$goods->picname}}"></a>
									</div>
								</li>
								@endforeach
							</ul>
			  ...
			  <br/>
						...
						<div class="tb-detail-list">
							...
							<div class="tb-detail-price">
								...
								<li class="price iteminfo_mktprice">
									<dt>原价</dt>
								<dd><em>¥</em><b class="sys_item_mktprice">{{$goods->original}}</b></dd>									
								</li>
								<div class="clear"></div>
							</div>
											...
											<form class="theme-signin" name="loginform" action="" method="post">
												...
												<div class="theme-signin-left">
													...
													<div class="theme-options">
														<div class="cart-title">类型</div>
														<ul>
															@foreach($goods->type as $type)
															<li class="sku-line " value="{{$type->id}}" onclick="price({{$type->price}},{{$type->original}})" >{{$type->name}}<i></i></li>
															@endforeach
															<br/>
														</ul>
													</div>
													<script>
														function price(price,original){
															// console.log(price,original);
															$('.sys_item_price').text(price);
															$('.sys_item_mktprice').text(original);
														}
													</script>
													{{-- <div class="theme-options">
													...
													<div class="theme-options">
														...
														<dd>
															...
															<input id="text_box" onkeyup="change(this,{{$goods->store}})" onchange="change(this,{{$goods->store}})" name="num" type="text" value="1" style="width:30px;" />
															<input id="add" class="am-btn am-btn-default" name="" type="button" value="+" />
														<span id="Stock" class="tb-hidden">库存<span class="stock">{{$goods->store}}</span>{{$goods->unit}}</span>
														</dd>
														<script>
						...
						<div class="pay">
							...
							<li>
								...
								<a id="LikBasket" title="加入购物车" href="javascript:;" onclick="Shopcart({{$goods->id}},$('#text_box').val(),{{$goods->sid}})"><i></i>加入购物车</a>
								</div>
							</li>
						</div>
...
<script>
	function Shopcart(id,num,sid){
		// console.log(id,num);
		if($('.sku-line.selected').val()==undefined){
			alert('请选择产品类型');return false;
		}else{
			var tid=$('.sku-line.selected').val();
		}
		$.ajax({
				...
				{{-- <div class="match">
					...
					<div class="match-comment">
						<ul class="like_list">
							<li>
								<div class="s_picBox">
									<a class="s_pic" href="#"><img src={{asset("Home/images/cp.jpg")}}></a>
							...
							<li>
								<div class="s_picBox">
									<a class="s_pic" href="#"><img src={{asset("Home/images/cp2.jpg")}}></a>
					    ...
					    <div class="mc"> 
						     <ul>					    
						     	<div class="mt">            
							      ...
							      <li class="first">
							      	<div class="p-img">                    
							      		<a  href="#"> <img class="" src={{asset("Home/images/browse1.jpg")}}> </a>               
							      	</div>
							...
							<div class="am-tabs-bd">
								...
								<div class="am-tab-panel am-fade am-in am-active">
									<div class="J_Brand">
										...
										<div class="clear"></div>
										<ul id="J_AttrUL">
										<li title="">产品类型:&nbsp;{{$goods->cname}}</li>
										<li title="">品牌:&nbsp;{{$goods->bname}}</li>
										<li title="">店铺:&nbsp;{{$goods->sname}}</li>
										<li title="">发布时间:&nbsp;{{$goods->time}} 年</li>
										<li title="">原料产地:&nbsp;{{$goods->company}}</li>
										<li title="">产品规格:&nbsp;{{$goods->weight}}</li>
										<li title="">产品简介：&nbsp;{{$goods->descr}}</li>
										</ul>
										<div class="clear"></div>
									</div>
									...
									<div class="details">
										...
										<div class="twlistNews">
											@foreach ($goods->detailsimg as $detailsimg)
											<img src="{{asset("uploads")}}/{{$detailsimg->img}}" />
											@endforeach
										</div>
									</div>
									<div class="clear"></div>
