...
<body>
   ...
   <div class="type_title">修改商品</div>
    ...
    <div class="formControls col-10"><input type="text" class="input-text" value="{{$goods->goods}}" placeholder="" id="" name="goods"></div>
		</div>
		{{-- <div class=" clearfix cl">
		...
		<div class=" clearfix cl">
			 ...
			 <div class="formControls col-2">
                 <select name="bid" id="">
                    @foreach ($brand as $brand)
                      @if($goods->bid == $brand['id'])
                        <option value="{{$brand['id']}}" selected>{{$brand['bname']}}</option>
                      @else
            ...
            <div class="Add_p_s">
                    ...
                    <div class="formControls col-2">
                        <select name="sid" id="">
                            ...
                            @if($goods->sid == $shop['id'])
                                <option value="{{$shop['id']}}" selected>{{$shop['sname']}}</option>
                            @else
                ...
                <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->specification}}" placeholder="请输入规格" id="" name="detail-specification"></div>
                       </div>
                <div class="Add_p_s">
                        <label class="form-label col-2">产品编号：</label>
                        <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->gnum}}" placeholder="请输入编号" id="" name="gnum"></div>
                        </div>
                        <div class="Add_p_s">
                         <label class="form-label col-2">产&nbsp;&nbsp;&nbsp;&nbsp;地：</label>	
                         <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->company}}" placeholder="请输入产地" id="" name="detail-company"></div>
                        </div>
             <div class="Add_p_s">
             <label class="form-label col-2">产品重量：</label>	
			 <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->weight}}" placeholder="请输入重量" id="" name="detail-weight" ></div>
			</div>
            
            <div class="Add_p_s">
             <label class="form-label col-2">展示价格：</label>	
			 <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->price}}" placeholder="请输入展示价格" id="" name="price" >元</div>
			</div>
            <div class="Add_p_s">
             <label class="form-label col-2">市场价格：</label>	
			 <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->original}}" placeholder="请输入原价" id="" name="original" >元</div>
			</div>
            <div class="Add_p_s">
                    <label class="form-label col-2">产品数量：</label>	
                    <div class="formControls col-2"><input type="text" class="input-text" value="{{$goods->store}}" placeholder="请输入数量" id="" name="store" ></div>
            </div>
		...
		<div class="clearfix cl">
			...
			<div class="formControls col-10">
				<input type="text" class="input-text" value="{{$goods->antishop}}" placeholder="请输入商品关键词" id="" name="antishop">
			</div>
		</div>
		<div class="clearfix cl">
			<label class="form-label col-2">内容简介：</label>
			<div class="formControls col-10">
				<textarea name="detail-descr" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="textarealength(this,200)">{{$goods->descr}}</textarea>
				...
				<div class="uploader-list-container"> 
					<div class="queueList">
						<div id="dndArea" class="placeholder">
                            <div id="filePicker-2"><img src="{{asset("/uploads")}}/{{$goods->picname}}" style='width:190px;height:140px;' /></div>
                            <input id="upload-input" style="position: absolute; top: 0; bottom: 0; left: 0;right: 0; opacity: 0;height:238px;width:790px;" name="pic[]" type="file" value="{{$goods->picname}}" accept="image/gif, image/jpg, image/png" onchange="img(this)" multiple/>
		...
		<div class="clearfix cl">
			<div class="Button_operation">
				<a href="Javascript:;" onclick="submit({{$goods->gid}});" class="btn btn-primary radius" type="submit"><i class="icon-save "></i>保存修改</a>
...
$.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
    function submit(id){
        ...
        $.ajax({ 
        url: "{{url('admin/Product/Product_update')}}"+'/'+id,
        type: 'POST', 
        data: formData, 
        contentType: false, 
        processData: false,
